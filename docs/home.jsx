/* global React, TOOLS, Page, PosterCta, siteMono, microMono, pageGutter, iosUrl, playUrl */
...
function Hero() {
  return (
    <section style={{ ...pageGutter, paddingTop: "var(--space-9)", paddingBottom: "var(--space-8)", borderBottom: "var(--rule)", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,1.1fr)", gap: "var(--space-8)", alignItems: "end", marginTop: "var(--space-7)" }}>
        <div style={{ display: "grid", gap: "var(--space-5)", maxWidth: "var(--measure)" }}>
          ...
          <div style={{ display: "flex", gap: "var(--space-3)", flexWrap: "wrap" }}>
            <HBadge store="ios" href={iosUrl("web-home-hero")} variant="solid" />
            <HBadge store="android" href={playUrl("web-home-hero")} variant="paper" />
          </div>
        </div>
        <HWave bars={40} height={210} seed={4} gap={7} playedTo={0.55} />
      </div>
      <HFeatureRow style={{ marginTop: "var(--space-8)" }} icons items={[
...
function ToolSection({ tool, flip }) {
  return (
    <section className={"mt-accent-" + tool.accent} style={{ ...pageGutter, paddingTop: "var(--space-8)", paddingBottom: "var(--space-8)", borderBottom: "var(--rule)" }}>
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,1fr)", gap: "var(--space-8)", alignItems: "center", direction: flip ? "rtl" : "ltr" }}>
        ...
        <HCard tone="card" style={{ direction: "ltr", aspectRatio: "4/3", display: "flex", flexDirection: "column", justifyContent: "center", gap: "var(--space-5)" }} pad="var(--space-6)">
          ...
          <HWave bars={28} height={130} seed={tool.id.length * 5} gap={6} playedTo={0.62} />
        </HCard>
      </div>
    </section>
  );
...
function PriceBand() {
  const plans = [
    ...
    { name: "Premium", head: "One payment. That's it.", items: ["All six stems, including piano and guitar", "Demixer without the weekly limit", "No subscription, no renewal — and it supports an independent developer"], cta: "See it in the app", variant: "paper", tone: "accent" },
  ];
  return (
    <section style={{ ...pageGutter, paddingTop: "var(--space-8)", paddingBottom: "var(--space-8)", borderBottom: "var(--rule)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: "var(--space-5)", maxWidth: "980px" }}>
        {plans.map((p) => (
